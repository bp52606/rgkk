...
contract Auction {
    ...
    function finalize() public {
        // TODO Your code here
        require(outcome == Outcome.SUCCESSFUL, "The outcome must be successful");
        require(sellerAddress!=address(0), "Seller must be defined");
        if(judgeAddress != address(0)) {
            require(msg.sender==judgeAddress && judgeAddress!=sellerAddress 
            || msg.sender==highestBidderAddress && highestBidderAddress!=sellerAddress, 
            "Only judge and highest bidder can finalize the auction");
        }
    ...
    function refund() public {
        // TODO Your code here
        require(outcome == Outcome.NOT_SUCCESSFUL, "Auction must not be successful for refunds");
        require(highestBidderAddress!=address(0), "Highest bidder must be defined");
        if(judgeAddress != address(0)) {
            require(msg.sender==sellerAddress && sellerAddress!=highestBidderAddress|| 
            msg.sender==judgeAddress && judgeAddress!=highestBidderAddress,
            "Only seller and judge can claim refunds");
        }
